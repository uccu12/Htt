...
// Load cookie từ cookies.json
let cookieHeader = '';
try {
    if (fs.existsSync('cookies.json')) {
        const cookies = JSON.parse(fs.readFileSync('cookies.json'));
        cookieHeader = cookies.map(c => `${c.name}=${c.value}`).join('; ');
        console.log('[>] Đã load cookie thành công.');
    }
} catch (e) {
    console.log('[!] Không thể load cookies.json, tiếp tục không dùng cookie.');
}
...
const secureOptions =
 ...
 if (process.argv.length < 7){console.log(` host time req thread proxy.txt`); process.exit();}
...
let headers = {
  ...
  "cookie": cookieHeader  // << Thêm dòng này để gửi Cookie
};
 const proxyOptions = {
     host: parsedProxy[0],
     port: ~~parsedProxy[1],
